...
class ModifiedTUsDataset(TUsDataset):
    def __init__(self, name):
        t0 = time.time()
        self.name = name

        # dataset = TUDataset(self.name, hidden_size=1)
        dataset = LegacyTUDataset(self.name, raw_dir='.', hidden_size=1)  # dgl 4.0
        ...
        #########
        # self.train = [[dataset[idx] for idx in self.all_idx['train'][split_num]] for split_num in range(1)]
        #########
...
class FusangTUsDataset(TUsDataset):
    def __init__(self, name):
        t0 = time.time()
        self.name = name
        ...
        self.test = self.format_dataset(dataset)
