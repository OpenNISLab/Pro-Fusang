...
"""
    GCN Nets for Fusang training phase
...
def train_val_pipeline(MODEL_NAME, DATASET_NAME, params, net_params, dirs):
    ...
    try:
        for split_number in range(10):
            ...
            # optimizer = optim.AdamW(model.parameters(), lr=params['init_lr'], weight_decay=7e-3)
            ...
            with tqdm(range(params['epochs'])) as t:
                for epoch in t:
                    ...
                    epoch_val_loss, epoch_val_acc, _ = evaluate_network(model, device, val_loader, epoch)
                    _, epoch_test_acc, _ = evaluate_network(model, device, test_loader, epoch)
            ...
            _, test_acc, _ = evaluate_network(model, device, test_loader, epoch)
            _, train_acc, _ = evaluate_network(model, device, train_loader, epoch)
...
def main():    
    ...
    parser.add_argument('--config', default='configs\TUs_graph_classification_GCN_HRRP_train.json')
